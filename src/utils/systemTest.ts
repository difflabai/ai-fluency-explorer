...
const testQuestionService = async (): Promise<SystemTestSuite> => {
  ...
  try {
    ...
    if (!questions || questions.length === 0) {
      ...
      const hasAllDifficulties = difficultyLevels.every(
        (level) => difficultyCounts[level] > 0
      );

      results.push({
        testName: 'Difficulty Distribution',
        status: hasAllDifficulties ? 'PASS' : 'WARNING',
        message: hasAllDifficulties
          ? 'All difficulty levels represented'
          : 'Some difficulty levels missing',
  ...
  try {
    ...
    if (perfectResult.overallScore === 5 && perfectResult.percentageScore === 100) {
      results.push({
        ...
        details: {
          score: perfectResult.overallScore,
          percentage: perfectResult.percentageScore,
        },
      });
    } else {
      results.push({
    ...
    if (
      partialResult.overallScore === expectedScore &&
      partialResult.percentageScore === expectedPercentage
    ) {
      results.push({
        ...
        details: {
          score: partialResult.overallScore,
          percentage: partialResult.percentageScore,
        },
      });
    } else {
      results.push({
    ...
    if (perfectResult.categoryScores && Array.isArray(perfectResult.categoryScores)) {
      ...
      const hasFluencyLevels = perfectResult.categoryScores.some((cat) =>
        ['Novice', 'Advanced Beginner', 'Competent', 'Proficient', 'Expert'].includes(
          cat.categoryName
        )
      );
  ...
  try {
    ...
    if (generationResult.count === testConfig.count) {
      results.push({
        ...
        details: {
          requestedCount: testConfig.count,
          actualCount: generationResult.count,
        },
      });
    } else {
      results.push({
...
export const generateTestReport = (testSuites: SystemTestSuite[]): string => {
  ...
  const warningTests = testSuites.reduce(
    (sum, suite) => sum + suite.results.filter((r) => r.status === 'WARNING').length,
    0
  );

  const overallStatus =
    failedTests > 0 ? 'FAIL' : warningTests > 0 ? 'WARNING' : 'PASS';
