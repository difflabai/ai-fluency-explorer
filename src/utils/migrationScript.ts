...
import {
  categories,
  customQuickAssessmentQuestions,
  sampleQuestions,
} from './testData';
...
export const migrateCategoriesToDatabase = async () => {
  try {
    ...
    const categoriesToInsert = categories.filter(
      (c) => !existingCategoryNames.has(c.id)
    );
    ...
    // Insert categories
    const { error } = await supabase.from('categories').insert(dbCategories);
...
export const migrateQuestionsToDatabase = async () => {
  try {
    ...
    const questionsToInsert = allQuestions.filter(
      (q) => !existingQuestionTexts.has(q.text)
    );
    ...
    const dbQuestions = questionsToInsert
      .map((q) => {
