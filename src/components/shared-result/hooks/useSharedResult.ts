    ...
    const loadSharedResult = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          console.log("Formatted category scores:", formattedCategoryScores);
          ...
          if (!formattedCategoryScores || formattedCategoryScores.length === 0) {
            console.log("No category scores found, generating fallback data");
          ...
          if (data.questions_snapshot) {
            ...
            const userAnswers = generateUserAnswers(data.questions_snapshot, data.percentage_score);
